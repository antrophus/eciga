			...
			<div id="list">
				<form action="${pageContext.request.contextPath}/admin/customer" method="get">
					<div class="period-filter">
						<span class="period-label">기간조회</span> <input type="date" name="startDate" class="date-input" value="${startDate}"> <input type="date"
							name="endDate" class="date-input" value="${endDate}"> <select class="form-control" name="status">
				...
				<div id="paging">
                    <ul>
                        <c:if test="${pMap.prev}">
                            <li><a href="${pageContext.request.contextPath}/admin/customer?crtpage=${pMap.startPageBtnNo-1}&keyword=${param.keyword}&status=${param.status}&startDate=${param.startDate}&endDate=${param.endDate}">◀</a></li>
                        </c:if>

                        <c:forEach begin="${pMap.startPageBtnNo}" end="${pMap.endPageBtnNo}" var="page">
                            <li class="${pMap.crtPage == page ? 'active' : ''}">
                                <a href="${pageContext.request.contextPath}/admin/customer?crtpage=${page}&keyword=${param.keyword}&status=${param.status}&startDate=${param.startDate}&endDate=${param.endDate}">${page}</a>
                            </li>
                        </c:forEach>

                        <c:if test="${pMap.next}">
                            <li><a href="${pageContext.request.contextPath}/admin/customer?crtpage=${pMap.endPageBtnNo+1}&keyword=${param.keyword}&status=${param.status}&startDate=${param.startDate}&endDate=${param.endDate}">▶</a></li>
                        </c:if>
                    </ul>
                </div>
